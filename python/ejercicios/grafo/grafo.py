...
class Grafo():
    ...
    def barrido_maravilla(self):
        for i in range(self.__elements):
            vertice1 = self.__elements[i]
            coincidences = []
        
            for j in range(i + 1, len(self.__elements)):
                vertice2 = self.__elements[j]
                if vertice1[0] == vertice2[0]:
                    coincidences.append(vertice2[0])
            
            if coincidences:
                print(f'Maravillas en {vertice1[0].pais} con el mismo pais:')
                print(vertice1[0])
    ...
    def dijkstra(self, origen, destino, criterio=None):
        ...
        while no_visitados.size() > 0:
            ...
            camino.apilar([vertice[1][0], vertice[0], vertice[2]])
